    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<List<Order>> GetActiveOrdersForCustomerAsync(Guid customerId)
        {
            var activeStatuses = new[]
            {
                ...
                OrderStatus.Ready,   OrderStatus.PickedUp,  OrderStatus.OnTheWay,
            };
        ...
        public async Task<bool> CanCustomerReviewOrderAsync(Guid orderId, Guid customerId)
        {
            ...
            if (order.Status != OrderStatus.Delivered && order.Status != OrderStatus.Completed) return false;
            if (order.Rating.HasValue) return false;
            return true;
        }
        ...
        public async Task<int> GetOrdersCountByDateRangeAsync(DateTime startDate, DateTime endDate)
        {
            return await _context.Orders
                .CountAsync(o => o.CreatedAt.Date >= startDate.Date &&
                                 o.CreatedAt.Date <= endDate.Date);
        }
    }
}