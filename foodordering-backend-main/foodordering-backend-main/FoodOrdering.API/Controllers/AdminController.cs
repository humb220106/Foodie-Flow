...
namespace FoodOrdering.API.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        /// <summary>GET /api/admin/reports/summary — all reports in one call</summary>
        [HttpGet("reports/summary")]
        public async Task<IActionResult> GetSummaryReport()
        {
            try
            {
                var result = await _adminService.GetSummaryReportAsync();
                return Ok(new { data = result });
            }
            catch (Exception ex)
            {
        ...
        /// <summary>GET /api/admin/reports/revenue?days=30</summary>
        [HttpGet("reports/revenue")]
        public async Task<IActionResult> GetRevenueReport([FromQuery] int days = 30)
        {
            try
            {
                if (days < 7) days = 7;
                if (days > 365) days = 365;
                var result = await _adminService.GetRevenueReportAsync(days);
                return Ok(new { data = result });
            }
            catch (Exception ex)
            {
        ...
        /// <summary>GET /api/admin/reports/orders?days=30</summary>
        [HttpGet("reports/orders")]
        public async Task<IActionResult> GetOrdersReport([FromQuery] int days = 30)
        {
            try
            {
                if (days < 7) days = 7;
                if (days > 365) days = 365;
                var result = await _adminService.GetOrdersReportAsync(days);
                return Ok(new { data = result });
            }
            catch (Exception ex)
            {
        ...
        /// <summary>GET /api/admin/reports/users?days=30</summary>
        [HttpGet("reports/users")]
        public async Task<IActionResult> GetUsersReport([FromQuery] int days = 30)
        {
            try
            {
                if (days < 7) days = 7;
                if (days > 365) days = 365;
                var result = await _adminService.GetUsersReportAsync(days);
                return Ok(new { data = result });
            }
            catch (Exception ex)
            {
        ...
        /// <summary>GET /api/admin/reports/restaurants</summary>
        [HttpGet("reports/restaurants")]
        public async Task<IActionResult> GetRestaurantsReport()
        {
            try
            {
                var result = await _adminService.GetRestaurantsReportAsync();
                return Ok(new { data = result });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error generating restaurants report");
                return StatusCode(500, new { message = "An error occurred while generating the report" });
            }
        }
